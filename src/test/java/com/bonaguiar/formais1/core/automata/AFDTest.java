...
public class AFDTest {
	...
	@Test
	public void testarGetMinimoDeAFMinimo() throws FormaisException {
		AFD afd = new AFD(new Alfabeto("ac")); // Autômato já é mínimo
		afd.addEstado("q0", false);
		afd.addEstado("q1", true);
		afd.addEstado("q2", true);
		afd.setEstadoInicial("q0");

		afd.addTransicao("q0", 'a', "q2");
		afd.addTransicao("q0", 'c', "q1");
		afd.addTransicao("q2", 'a', "q2");
		afd.addTransicao("q2", 'c', "q1");
		AFMin min = afd.getAFMin();
		...
		assertEquals(Arrays.asList("q0", "q1", "q2"), min.getEstados());
		assertEquals(3, min.getEstados().size());
