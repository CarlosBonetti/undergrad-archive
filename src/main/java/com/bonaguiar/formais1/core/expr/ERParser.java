...
public class ERParser {
	...
	/**
	 * Transforma a expressão com símbolos especiais para uma expressão equivalente com símbolos primitivos
	 * Exemplo: 'a+' = 'aa*'
	 *
	 * @param expr
	 * @return
	 */
	public static String traduzirSimbolosEspeciais(String expr) {
		while (expr.indexOf("+") != -1) {
			int pos = expr.indexOf("+");

			String duplicate = expr.substring(pos - 1, pos);

			// Para expressões regulares do tipo '(ab)+', devemos duplicar todo o conteúdo dentro do parêntese
			if (duplicate.equals(")")) {
				int depth = 0; // nível atual de parênteses
				for (int i = pos - 1; i >= 0; i--) {
					...
					if (depth == 0) {
						duplicate = expr.substring(i, pos);
						break;
					}
				}
			}

			expr = expr.replaceFirst("\\+", duplicate + "*");
		}
